...
- **Qué precio se usa según la hora (21/09/2026, dos bugs reales que aparecieron con datos de otra hora):**
  - **Fuera de rueda (antes de las 11, después de las 17, fin de semana, feriado) se usa el ÚLTIMO operado, no la punta vendedora** (`FCNLetras.enRueda`). A las 23 hs el libro queda con puntas viejas y spreads anchos (S30S6: compra 116,51 / venta 117,39 / último 116,92) y la punta daba 0,02% de ganancia en vez de 0,43%. Dentro de la rueda sigue la punta vendedora (lo que pagaría un comprador).
  - **El precio guardado en el Monitor solo es respaldo si tiene ≤ 4 días** (`MAX_EDAD_MONITOR`; letras: fecha de la fila "compra" → campo `fechaPrecio`/`fp` del JSON; bonos: `liqMonitor`). **Antes de la apertura (~10 hs) BYMA devuelve CERO filas** en `lebacs` y `public-bonds`; con un Monitor de 6 días las letras mostraban TNA 41% (real ~17%) porque en 8 días 0,7% de precio cambia toda la tasa. Ahora la fila queda sin precio y la pantalla dice "BYMA todavía no publicó cotizaciones (abre a las 11 hs)…". Si el navegador tiene precios guardados de la rueda anterior se usan rotulados "(desactualizado)" — es el respaldo bueno. Un JSON viejo sin `fp` no habilita el respaldo (más seguro).
  - Gotcha de pruebas: el panel del navegador cachea `fcn-*.js` por nombre; al probar cambios agregar `?v=…` o no se ve el módulo nuevo.
